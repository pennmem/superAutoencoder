...
def get_prior_sample_weights(y_prior, y_current):
    ...
    n_total = n_prior + n_current
    n_prior = n_total/n_prior/2.0
    n_current = n_total/n_current/2.0

    weights = np.concatenate([np.repeat(n_prior, len(y_prior)), np.repeat(n_current, len(y_current))])
...
def scale_sessions(pow_mat, event_sessions, pow_mat_ref = None, event_sessions_ref = None):
    ...
    for sess in sessions:
        ...
        pow_mat[sess_event_mask] = (pow_mat[sess_event_mask] - mins)/maxs
    return pow_mat
...
def opt_params(X,y, session, list_session, classifier_name, ind_params, search_method, type_of_data, feature_select):
    ...
    if classifier_name == 'L2':
         ...
         if type_of_data == 'long':
             space4classifier ={
             'C': hp.loguniform('C',-20,-10)}
         elif feature_select == 1:
             space4classifier ={
             'C': hp.loguniform('C',-20,-10)}
         else:
             space4classifier ={
             'C': hp.loguniform('C',-10,-5)}
    ...
    if search_method == 'rand':
        best_params = fmin(f, space4classifier, algo = hyperopt.rand.suggest, max_evals = 20)

    if search_method == 'tpe':
        best_params = fmin(f, space4classifier, algo = hyperopt.tpe.suggest, max_evals = 20)

    return space_eval(space4classifier,best_params)


def run_loso_xval(dataset, classifier_name = 'current', search_method = 'rand', type_of_data = 'normal',  feature_select= 0,  adjusted = 1, C_factor = 1.0):
    ...
    for i,sess in enumerate(sessions):
        ...
        if classifier_name == 'noisy_L2':
            ind_params = {'learning_rate':0.01, 'max_iter':5000, 'batch_size':10, 'momentum':0.8}
            best_params = opt_params(insample_pow_mat,insample_recalls, insample_sess, insample_list, classifier_name, ind_params, search_method, type_of_data, feature_select)
            ind_params.update(best_params)
