...
def opt_params(X,y, session, list_session, classifier_name, ind_params, search_method, type_of_data, feature_select):
    ...
    if search_method == 'rand':
        best_params = fmin(f, space4classifier, algo = hyperopt.rand.suggest, max_evals = 100)

    if search_method == 'tpe':
        best_params = fmin(f, space4classifier, algo = hyperopt.tpe.suggest, max_evals = 100)
